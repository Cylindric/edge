        ...
        if ($char_is_owned) {
            ...
            public function create() {
                ...
                if ($this->request->is('post')) {
                    ...
                    $s = $this->Characters->Species->get($data['species_id']);
                    $data['xp'] = [['value' => $s->base_xp, 'note' => 'Species starting XP']];

                    $character = $this->Characters->newEntity($data, ['associated' => 'Xp']);
                    $character->user_id = $this->Auth->user('id');

                    // Set values from Species
                    $character->stat_ag = $s->stat_ag;
                    $character->stat_br = $s->stat_br;
                    $character->stat_cun = $s->stat_cun;
                    $character->stat_int = $s->stat_int;
                    $character->stat_pr = $s->stat_pr;
                    $character->stat_will = $s->stat_will;

                    if ($this->Characters->save($character)) {
                        // Get the new Character, with associations
                        $character = $this->Characters->get($character->id, ['contain' => ['Species', 'Xp']]);
